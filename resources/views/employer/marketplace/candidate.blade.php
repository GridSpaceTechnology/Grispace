            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 sticky top-24">
                    ...
                    <div class="space-y-3">
                        ...
                        <details class="mt-3">
                            <summary class="text-xs text-slate-400 hover:text-slate-600 cursor-pointer select-none text-center">
                                Was this recommendation helpful?
                            </summary>
                            <form method="POST" action="{{ route('employer.feedback.store', ['candidate' => $candidate->id]) }}" class="mt-2 space-y-1.5">
                                @csrf
                                @foreach(['relevant_candidate' => 'Relevant candidate', 'skills_mismatch' => 'Skills mismatch', 'experience_mismatch' => 'Experience mismatch', 'role_mismatch' => 'Role mismatch', 'not_interested' => 'Not what we need'] as $value => $label)
                                    <button type="submit" name="feedback" value="{{ $value }}"
                                            class="w-full px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded text-[11px] transition-colors">
                                        {{ $label }}
                                    </button>
                                @endforeach
                            </form>
